const extranetUsername = process.env.EXTRANET_USERNAME
const extranetPassword = process.env.EXTRANET_PASSWORD
...
export class ExtranetClient implements MatchGateway {
  ...
  async getToken() {
    try {
      if (!this.token || !this.tokenExpiresAt || Date.now() > this.tokenExpiresAt) {
        ...
        if (!res.ok) {
          throw new Error(`Erreur lors de la récupération du token: ${res.status} ${res.statusText}`)
        }
  ...
  async getPoulesIds(): Promise<number[]> {
    ...
    const endpoint =
      this.baseEndpoint + "/competition/getEngagementsParOrganisme.ws?idOrganisme=" + club.clubId.toString()
  ...
  async getCompetitions() {
    ...
    try {
      const endpoint = this.baseEndpoint + "/competition/getCompetitionParOrganisme.ws?codeOrganisme=" + club.id
      const token = await this.getToken()
      const response = await fetch(endpoint, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      this.competitions = data.map((compet) => ({ id: compet.id, label: compet.code, poules: compet.poules }))
      return this.competitions
    } catch (error) {
  ...
  async getClubs() {
    ...
    try {
      const token = await this.getToken()
      const ids = await this.getCompetitions().then((competitions) => competitions.map((compet) => compet.id))

      const clubs = await Promise.all(
        ids.map(async (id) => {
          try {
            const endpoint = this.baseEndpoint + "/competition/getClassementEquipes.ws?idCompetition=" + id
            const response = await fetch(endpoint, {
              headers: {
                Authorization: `Bearer ${token}`,
              },
    ...
    try {
      ...
      const allMatchsFromClubs = await Promise.all(
        poulesIds.map(async (poule) => {
          const endpoint = this.baseEndpoint + "/competition/getRencontresParPoule.ws?idPoule=" + poule
          const res = await fetch(endpoint, {
            headers: {
              Authorization: `Bearer ${token}`,
            },
      ...
      const matchsForMyClub = flatMatchs.filter(
        (match) => match.idOrganismeEquipe1 === club.clubId || match.idOrganismeEquipe2 === club.clubId
      )
  ...
  async getAdherents(): Promise<Adherent[]> {
    ...
    if (!res.ok) {
      throw new Error("Failed to fetch adherents", { cause: { statusText: res.statusText, status: res.status } })
    }
