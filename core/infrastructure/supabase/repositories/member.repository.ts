  ...
  implements MemberRepository
{
  ...
  async findByRole(role: MemberRole): Promise<MemberEntity[]> {
    ...
    const { data, error } = await client.from(this.tableName).select("*").filter("role", "cs", `{${role}}`)
  ...
  async findDirigeants(): Promise<MemberEntity[]> {
    const client = await this.getClient()
    const dirigeants = [MemberRole.President, MemberRole.VicePresident, MemberRole.Secretary, MemberRole.Treasurer]
    const { data, error } = await client.from(this.tableName).select("*").in("role", dirigeants)
    if (error) throw error
    return data.map((item) => toDomain(item))
  }
  ...
  async findMembers(): Promise<MemberEntity[]> {
    const client = await this.getClient()
    const { data, error } = await client.from(this.tableName).select("*").not("role", "cs", "{coach}")
    if (error) throw error
    return data.map((item) => toDomain(item))
  }
}
