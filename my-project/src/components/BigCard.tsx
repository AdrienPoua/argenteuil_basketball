...
type BigCardProps = { url: string; img: string; title: string; date: string };

export const BigCard = ({ url, img, title, date }: Readonly<BigCardProps>) => {
    ...
    >
      <div className='relative h-full w-full'>
        ...
        <div className='absolute inset-x-0 bottom-5 text-white gap-4'>
          ...
          <div className='text-sm ps-5 pb-5'>{date}</div>
        </div>
      </div>
    </a>
  );
