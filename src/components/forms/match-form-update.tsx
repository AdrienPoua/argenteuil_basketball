...
export default function UpdateAmicalMatchForm({ currentMatch, actions, currentTeam }: Readonly<PropsType>) {
  ...
  const mutation = useMutation({
    mutationFn: async (data: UpdateMatchFormValues) => {
      await updateMatch({ ...data, id: currentMatch.id })
    },
    onSuccess: () => {
      form.reset()
      actions.success()
      ...
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        <div className="bg-muted mb-6 rounded-md p-4">
          ...
          <div className="mt-2 text-sm">
            ...
            <p>
              <strong>Score:</strong> {`${currentMatch.resultatEquipe1} - ${currentMatch.resultatEquipe2}`}
            </p>
            <p>
            ...
            {currentTeam && (
              <>
                ...
                <p>
                  ...
                  {currentTeam?.coachs.map((coach) => `${coach.first_name} ${coach.last_name}`).join(", ")}
                </p>
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input {...field} disabled={mutation.isPending || !currentMatch.isAmical} required />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input type="date" {...field} disabled={mutation.isPending || !currentMatch.isAmical} required />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input type="number" {...field} disabled={mutation.isPending || !currentMatch.isAmical} required />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
