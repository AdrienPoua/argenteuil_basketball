...
export default function Footer() {
  return (
    <footer className="border-t border-primary font-secondary text-white">
      <div className="container mx-auto px-4 py-16">
        <div className="grid grid-cols-1 gap-8 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-4">
            <Link href="/" className="group flex items-center space-x-3">
              ...
              />
              <div>
                <h3 className="bg-primary bg-clip-text text-lg font-bold text-transparent">
                  {club.name}
                </h3>
                <p className="text-xs">Basketball Club</p>
              </div>
            </Link>
            <p className="text-sm leading-relaxed">
              Passionnés de basketball depuis des années, nous formons et accompagnons les joueurs
              de tous niveaux dans un esprit de fair-play et de dépassement de soi.
            </p>
          ...
          {/* Navigation rapide */}
          <div className="space-y-4">
            <h4 className="font-main text-sm font-semibold uppercase tracking-wide text-primary">
              Navigation
            </h4>
            <nav className="space-y-3">
              <FooterLink href="/club/equipes">Nos équipes</FooterLink>
              <FooterLink href="/plannings/matchs">Calendrier</FooterLink>
              <FooterLink href="/inscriptions/guide">Rejoindre le club</FooterLink>
          ...
          <div className="space-y-4">
            ...
            <nav className="space-y-3">
              <FooterLink href="/inscriptions/tarifs">Tarifs</FooterLink>
          ...
          <div className="space-y-4">
            ...
            <div className="pt-4">
              ...
              >
                <Link href="/inscriptions/guide">Rejoindre le club</Link>
              </Button>
            </div>
          </div>
        </div>
        ...
        <div className="flex flex-col items-center justify-between space-y-4 md:flex-row md:space-y-0">
          <div className="flex flex-col items-center space-y-2 text-sm text-foreground md:flex-row md:space-x-6 md:space-y-0">
            <p>
              © 2024{' '}
              <Link href={`https://www.adrienpoua.fr/`} className="hover:text-primary">
                Adrien POUA
              </Link>
          ...
          <div className="group flex items-center space-x-2">
            <span className="text-sm text-foreground">Créé avec</span>
...
function SocialButton({ href, icon }: Readonly<{ href: string; icon: React.ReactNode }>) {
  return (
    <Link
      href={href}
      className="flex h-10 w-10 items-center justify-center rounded-full bg-primary text-white transition-all duration-300 hover:scale-110 hover:bg-primary/10 hover:text-primary"
    >
      {icon}
    </Link>
  )
...
function FooterLink({ href, children }: Readonly<{ href: string; children: React.ReactNode }>) {
  return (
    <Link
      href={href}
      className="group block text-sm transition-colors duration-300 hover:text-foreground"
    >
...
function ContactInfo({
  ...
  return (
    <Link
      href={href}
      className="group flex items-center space-x-3 text-sm transition-colors duration-300 hover:text-foreground"
    >
