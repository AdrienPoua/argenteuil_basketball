...
export function TeamSchedules({ teams }: Readonly<TeamSchedulesProps>) {
  return (
    <div className='grid gap-6 p-5 md:grid-cols-2 lg:grid-cols-3'>
      {teams.map((team) => (
        <ScaleFromBottom key={team.id}>
          <Card
            className={`group relative min-h-[400px] cursor-pointer overflow-hidden rounded-lg bg-cover bg-center p-3 font-secondary transition-all duration-300 hover:scale-105 hover:transform`}
