  ...
  return (
    <div className='container mx-auto flex flex-col gap-8 px-4'>
      {matchsByChampionnat.map((matchs, index) => {
        ...
        return (
          <ChampionnatMatchs key={matchs[0].championnat} matchs={matchs} />
        );
      })}
    </div>
  );
  ...
  return (
    <div className='space-y-4'>
      ...
      <div className="relative w-full">
        ...
        {scrollable && (
          <>
            <div className="absolute -left-10 md:-left-20 top-1/2 -translate-y-1/2 z-10 hidden md:block">
              <button 
                onClick={() => scroll('left')} 
                disabled={!canScrollLeft}
                className={`rounded-full bg-primary/10 p-2 text-primary hover:bg-primary/20 ${!canScrollLeft ? 'opacity-50 cursor-not-allowed' : ''}`}
            ...
            <div className="absolute -right-10 md:-right-20 top-1/2 -translate-y-1/2 z-10 hidden md:block">
              <button 
                onClick={() => scroll('right')} 
                disabled={!canScrollRight}
                className={`rounded-full bg-primary/10 p-2 text-primary hover:bg-primary/20 ${!canScrollRight ? 'opacity-50 cursor-not-allowed' : ''}`}
        ...
        >
          {matchs.map((match) => (
            <Suspense fallback={<SkeletonCard key={`skeleton-${match.id}`} />} key={match.id}>
              <div className='w-[220px] sm:w-[260px] md:w-[280px] lg:w-[300px] flex-shrink-0 overflow-hidden rounded-lg'>
                <Card match={match} />
              </div>
            </Suspense>
          ))}
