...
export const components: PortableTextComponents = {
  types: {
    ...
    callout: ({ value }) => {
      ...
      return (
        <div className={`my-6 rounded-lg border-l-4 p-4 ${styles[icon as keyof typeof styles] || styles.info}`}>
          <div className='flex items-start space-x-3'>
            <div className='flex-shrink-0 pt-0.5'>{icons[icon as keyof typeof icons] || icons.info}</div>
  ...
  marks: {
    ...
    highlight: ({ children }) => <span className='bg-yellow-100 px-1 text-yellow-800'>{children}</span>,
  },
  block: {
    h1: ({ children }) => (
      <h1
        className={cn('mb-6 mt-12 text-4xl font-bold tracking-tight text-gray-900', 'scroll-m-20 lg:text-5xl')}
    ...
    h3: ({ children }) => (
      <h3
        className={cn('mb-4 mt-8 text-2xl font-semibold tracking-tight text-gray-900', 'scroll-m-20')}
        id={typeof children === 'string' ? children.toLowerCase().replace(/\s+/g, '-') : undefined}
      >
        {children}
      </h3>
    ...
    h4: ({ children }) => (
      <h4
        className={cn('mb-4 mt-6 text-xl font-semibold tracking-tight text-gray-900', 'scroll-m-20')}
        id={typeof children === 'string' ? children.toLowerCase().replace(/\s+/g, '-') : undefined}
      >
        {children}
      </h4>
    ...
    normal: ({ children }) => (
      <p className='mb-4 font-secondary leading-7 text-gray-700 [&:not(:first-child)]:mt-6'>{children}</p>
    ),
    blockquote: ({ children }) => (
  ...
  list: {
    ...
    number: ({ children }) => (
      <ol className='my-6 ml-6 list-decimal font-secondary text-gray-700 [&>li]:mt-2'>{children}</ol>
    ),
  },
  listItem: {
