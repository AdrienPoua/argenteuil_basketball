...
const ClickableImage = ({ value }: { value: any }) => {
  ...
  return (
    <>
      <figure 
        className="my-10 group relative cursor-zoom-in overflow-hidden rounded-lg transition-all duration-300"
      ...
      >
        <div className="relative w-full overflow-hidden">
          ...
          <div className="relative w-full">
            <Image
              src={imageUrl}
              alt={value.alt || ''}
              className="w-full h-auto max-h-[800px] object-contain mx-auto transition-transform duration-500 ease-in-out group-hover:scale-105"
              height={800}
              width={1200}
            />
        ...
        {hasCaption && (
          <figcaption className="bg-gray-50 p-3 text-center text-sm text-gray-600 italic">
            {value.caption}
          </figcaption>
        )}
      </figure>
...
export const components: PortableTextComponents = {
      ...
      return (
        <div className={`my-6 rounded-lg border-l-4 p-4 ${styles[icon as keyof typeof styles] || styles.info}`}>
          <div className="flex items-start space-x-3">
            ...
            <div className="flex-1 font-secondary text-sm">
              {content}
            </div>
          </div>
        </div>
  ...
  list: {
    bullet: ({ children }) => (
      <ul className="my-6 ml-6 list-disc font-secondary text-gray-700 [&>li]:mt-2">
        {children}
      </ul>
    ),
    number: ({ children }) => (
