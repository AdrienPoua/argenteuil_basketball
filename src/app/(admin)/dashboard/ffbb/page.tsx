...
export default function Page() {
  ...
  return (
    <Tabs defaultValue={competitions[0].id.toString()} className='size-full max-w-full overflow-x-hidden'>
      <Button onClick={() => clubsUpdate()} className='mx-auto my-10 flex' disabled={transfering}>
        {' '}
        Mettre a jour les clubs
      </Button>
      <Button onClick={() => matchsUpdate()} className='mx-auto my-10 flex' disabled={transfering}>
        {' '}
        Mettre a jour les matchs
      </Button>
      <TabsList className='mx-auto mb-10 flex size-fit flex-wrap'>
        {competitions.map((compet) => (
          <TabsTrigger key={compet.id} value={compet.id.toString()}>
            {compet.code}
          </TabsTrigger>
        ))}
      </TabsList>
      {competitions?.map((compet) => {
        const filteredMatchs =
          matchs?.filter((match) => compet.poules.some((poule) => poule.id === match.idPoule)) || [];
        return (
          <TabsContent key={compet.id} value={compet.id.toString()} className='grid grid-cols-1 gap-5 md:grid-cols-5'>
